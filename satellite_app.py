...
satellites = [EarthSatellite(l1, l2, name, ts) for l1, l2, name in tle_lines]


days = 31
hours = np.arange(0, 24, 1)
all_times = ts.utc(2024, 8, np.repeat(np.arange(1, days+1), len(hours)), 0, np.tile(hours, days))


xs_list, ys_list, zs_list = [], [], []
for sat in satellites:
    positions = sat.at(all_times).position.km
    xs, ys, zs = positions
    xs_list.append(xs)
    ys_list.append(ys)
    zs_list.append(zs)
...
for i in range(len(satellites)):
    orbit_lines.append(go.Scatter3d(
        ...
        name=f"{satellites[i].name} Orbit",
        opacity=0.4,  
        showlegend=True
    ))
...
for k in range(len(all_times)):
    ...
    for i in range(len(satellites)):
        frame_data.append(go.Scatter3d(
            x=[xs_list[i][k]], y=[ys_list[i][k]], z=[zs_list[i][k]],
            mode='markers',
...
for i in range(len(satellites)):
    data_init.append(go.Scatter3d(
        x=[xs_list[i][0]], y=[ys_list[i][0]], z=[zs_list[i][0]],
        mode='markers',
...
fig = go.Figure(
    data=data_init+[earth] + orbit_lines,
    layout=go.Layout(
        title="Satellite Orbit Animation",
        scene=dict(xaxis=dict(range=[-10000, 10000]),
